...
window.EkraahNotifications = (() => {
  ...
  async function open() {
    ...
    document.addEventListener('click', function notifOutsideClick(e) {
      const panel = document.getElementById('notification-panel');
      const overlay = document.getElementById('notification-overlay');
      const bellBtn = document.getElementById('notification-bell-btn');
      if (panel && panel.classList.contains('open') && !panel.contains(e.target) && overlay && !overlay.contains(e.target) && bellBtn && !bellBtn.contains(e.target)) {
        close();
...
window.EkraahChatbot = (() => {
  ...
  const PRESET_OPTIONS = [
    {
      ...
      response: 'Navigate to the Services page from the bottom navigation bar, search for the service you need, and click on it to start your application.',
      action: null
    },
    {
    ...
    },
    {
      ...
      response: "You can check the status of your applications in the 'My Applications' section. Each application shows its current stage.",
      action: null,
    ...
    },
    {
      ...
      response: 'For support, you can email us at support@ekraah.gov.in or call our helpline at 1800-XXX-XXXX (toll-free).',
      action: null
    }
  ];
  ...
  function initFabListener() {
    ...
    // Close any previously open chatbot on page navigation
    isOpen = false;
...
window.EkraahModal = (() => {
  ...
  function show({ title = '', content = '', size = 'md', onClose = null, footer = '' } = {}) {
    ...
    // Force a layout reflow so the browser processes the initial (hidden) state
    // before we add the .active class. Without this, the CSS transition may not
    // fire and the modal-dialog can end up invisible while the backdrop is visible.
    const _backdrop = document.getElementById('modal-backdrop');
    const _wrapper = document.getElementById('modal-wrapper');
    // Force reflow by reading a layout property
    if (_backdrop) void _backdrop.offsetHeight;

    // Trigger animations using double-rAF for maximum reliability
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (_backdrop) _backdrop.classList.add('active');
        if (_wrapper) _wrapper.classList.add('active');
      });
    });
...
window.EkraahComponents = (() => {
  ...
  function statusTrack(stages, currentStage, status) {
    ...
    const stageItems = stages.map((stage, index) => {
      ...
      const connectorHtml = index < stages.length - 1 ? `
        <div style="
          flex: 1;
          height: 2px;
