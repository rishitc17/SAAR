...
window.EkraahChatbot = (() => {
  ...
  function render() {
    return `
      ...
      <button id="chatbot-fab" class="chatbot-fab" aria-label="Open Assistant">
        <i class="fas ${State()?.get('chatbotOpen') ? 'fa-times' : 'fa-headset'}" style="font-size:24px;"></i>
      </button>

      <!-- Chatbot Panel -->
      <div id="chatbot-panel" class="chatbot-panel ${State()?.get('chatbotOpen') ? 'open' : ''}" style="display:${State()?.get('chatbotOpen') ? 'flex' : 'none'};">
  ...
  function initFabListener() {
    ...
    // Restore open state if chatbot was open before navigation
    if (State()?.get('chatbotOpen')) {
      const panel = document.getElementById('chatbot-panel');
      if (panel) {
        isOpen = true;
        panel.style.display = 'flex';
