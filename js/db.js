...
const EkraahDB = (() => {
    ...
    async function getNotifications(userId) {
        const { data, error } = await window.EkraahDB.from('notifications')
            .select('*')
            .eq('user_id', userId)
    ...
    async function markNotificationRead(notifId) {
        const { data, error } = await window.EkraahDB.from('notifications').update({ read: true }).eq('id', notifId);
        if (error) throw error;
        return data;
    }
    ...
    async function createNotification(userId, title, message, type, relatedAppId = null) {
        const { data, error } = await window.EkraahDB.from('notifications').insert({
            user_id: userId,
            title,
            message,
            type,
    ...
    async function createApplication(citizenId, typeId, formData, totalStages, status = 'submitted') {
        const { data, error } = await window.EkraahDB.from('applications')
            .insert({
    ...
    async function getDepartmentApplications(department) {
        ...
        try {
            ...
            for (const app of data || []) {
                const { data: reviews } = await window.EkraahDB.from('application_stage_reviews')
                    .select('stage_number')
    ...
    async function getDepartmentAppsSimple(department) {
        ...
        try {
            ...
            const { data: reviews, error: revError } = await window.EkraahDB.from('application_stage_reviews')
            ...
            const { data: apps, error: appError } = await window.EkraahDB.from('applications')
                .select('*, application_types (name, slug, icon, color)')
                .in('id', appIds)
                .order('created_at', { ascending: false });
            if (appError) {
    ...
    async function createStageReviews(applicationId, stages) {
        ...
        const { data, error } = await window.EkraahDB.from('application_stage_reviews').insert(reviews);
        if (error) throw error;
        return data;
    }

    async function approveStage(reviewId, reviewerId) {
        const { data, error } = await window.EkraahDB.from('application_stage_reviews')
            .update({
                status: 'approved',
    ...
    async function addWorkNote(applicationId, authorId, note) {
        const { data, error } = await window.EkraahDB.from('work_notes').insert({
            application_id: applicationId,
            author_id: authorId,
            note,
        });
    ...
    async function getWorkNotes(applicationId) {
        const { data, error } = await window.EkraahDB.from('work_notes')
    ...
    async function getDocuments(citizenId) {
        const { data, error } = await window.EkraahDB.from('documents')
    ...
    async function issueDocument(citizenId, applicationId, docType, details = {}) {
        ...
        const { data, error } = await window.EkraahDB.from('documents').insert({
            citizen_id: citizenId,
            application_id: applicationId,
    ...
    async function getAvailableCases(specialization) {
        ...
        const { data: appTypes } = await window.EkraahDB.from('application_types')
            .select('id')
    ...
    async function acceptCase(lawyerId, applicationId) {
        ...
        const { data: app } = await window.EkraahDB.from('applications')
            .update({
        ...
        await window.EkraahDB.from('lawyer_cases').insert({
            lawyer_id: lawyerId,
            application_id: applicationId,
            status: 'active',
        });
    ...
    async function completeCase(lawyerId, applicationId) {
        await window.EkraahDB.from('lawyer_cases')
            .update({
                status: 'completed',
        ...
        await window.EkraahDB.from('applications')
            .update({
                status: 'completed',
                updated_at: new Date().toISOString(),
            })
    ...
    async function getLawyerCases(lawyerId) {
        const { data, error } = await window.EkraahDB.from('lawyer_cases')
            .select(
                '*, applications (*, application_types (name, slug, icon, color), profiles:citizen_id (full_name, email))',
            )
            .eq('lawyer_id', lawyerId)
    ...
    async function getApplicationTypes() {
        const { data, error } = await window.EkraahDB.from('application_types')
            .select('*')
    ...
    async function getApplicationTypeBySlug(slug) {
        const { data, error } = await window.EkraahDB.from('application_types').select('*').eq('slug', slug).single();
        if (error) throw error;
        return data;
    }
    ...
    async function getDashboardStats(department) {
        ...
        try {
            const { data: reviews, error } = await window.EkraahDB.from('application_stage_reviews')
            ...
            const { data: approvedReviews } = await window.EkraahDB.from('application_stage_reviews')
    ...
    async function getAllApplicationsCount() {
        const { count } = await window.EkraahDB.from('applications').select('*', { count: 'exact', head: true });
        return count || 0;
    }
    ...
    function subscribeToNotifications(userId, callback) {
        ...
        const channel = window.EkraahDB.channel(channelName)
            .on(
                'postgres_changes',
                {
                    event: 'INSERT',
