...
(function () {
  ...
  async function renderAppDetail(params) {
    ...
    try {
      ...
      // Fetch government official details for all note authors to get their ACTUAL departments
      const authorIds = [...new Set((workNotes || []).map(n => n.author_id).filter(Boolean))];
      const officialDeptMap = {};
      if (authorIds.length > 0) {
        try {
          ...
          if (officials) {
            officials.forEach(o => { officialDeptMap[o.user_id] = o.department; });
          }
        } catch (err) {
          console.warn('Could not fetch official departments for work notes:', err);
        }
      }
      ...
      // Use both stageReviews data AND application.current_stage as a fallback
      // so that stages completed by OTHER departments still show a green tick
      // even if RLS prevents us from reading their review data.
      const statusTrackStages = workflowStages.map(ws => {
        const review = stageReviews.find(r => r.stage_number === ws.stage);
        let stageStatus = review?.status || 'pending';

        // Fallback: if no review data found but the stage is before current_stage,
        // it must have been approved (the application moved past it).
        if (stageStatus === 'pending' && ws.stage < application.current_stage) {
          stageStatus = 'approved';
        }
        // Also if the application is fully approved, all stages are approved
        if (stageStatus === 'pending' && application.status === 'approved') {
          stageStatus = 'approved';
        }
      ...
      const notesHtml = (workNotes || []).length > 0
        ? workNotes.map(note => {
            ...
            // Use actual department from government_officials, not the viewer's department
            const authorDept = officialDeptMap[note.author_id] || '';
            return `
      ...
      if (isCurrentReviewerDept && currentStageReview && application.status !== 'approved' && application.status !== 'rejected') {
        ...
        const approveLabel = isLastStage
          ? '<i class="fas fa-file-signature" style="margin-right:8px;"></i>Approve & Issue Document'
      ...
      if (addNoteBtn && noteInput) {
        addNoteBtn.addEventListener('click', async () => {
          ...
          try {
            ...
            // Append the new note directly to the DOM instead of full page refresh
            const notesList = document.getElementById('gov-work-notes-list');
            if (notesList) {
              // Remove "No work notes yet." placeholder if present
              const noNotesEl = notesList.querySelector('p');
              if (noNotesEl && noNotesEl.textContent.includes('No work notes')) {
                noNotesEl.remove();
              }
              const profile = State().get('profile');
              const authorName = profile?.full_name || 'Official';
              const authorDept = myDepartment || '';
              const noteEl = document.createElement('div');
              noteEl.style.cssText = 'padding:var(--space-3) 0; border-bottom:1px solid var(--border-light);';
              noteEl.innerHTML = `
                <div style="display:flex; align-items:center; gap:var(--space-2); margin-bottom:var(--space-1);">
                  <span style="font-size:var(--text-sm); font-weight:var(--font-semibold); color:var(--text-primary);">${authorName}</span>
                  ${authorDept ? `<span style="font-size:var(--text-xs); color:var(--text-light); background:var(--bg-page); padding:2px 6px; border-radius:var(--radius-full);">${authorDept}</span>` : ''}
                  <span style="font-size:var(--text-xs); color:var(--text-light); margin-left:auto;">Just now</span>
                </div>
                <p style="font-size:var(--text-sm); color:var(--text-secondary); line-height:var(--leading-normal);">${noteText.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>
              `;
